...
public class TestSchemaUpdateSync {
  ...
  public void testAddTopLevelPrimitives() {
    ...
    Schema applied = new SchemaUpdate(new Schema(), 0).updateSchema(newSchema).apply();
  ...
  public void testAddTopLevelStructOfPrimitives() {
    for(PrimitiveType primitiveType : primitiveTypes()) {
      ...
      Schema applied = new SchemaUpdate(new Schema(), 0).updateSchema(currentSchema).apply();
  ...
  public void testAddNestedPrimitive() {
    for(PrimitiveType primitiveType : primitiveTypes()) {
      ...
      Schema newSchema = new Schema(optional(1, "aStruct", Types.StructType.of(
          optional(2, "primitive", primitiveType))));
      Schema applied = new SchemaUpdate(currentSchema, 1).updateSchema(newSchema).apply();
      Assert.assertEquals(newSchema.asStruct(), applied.asStruct());
    }
  }
  ...
  public void testDetectInvalidTopLevelList() {
    ...
    new SchemaUpdate(currentSchema, 2).updateSchema(newSchema).apply();
  }
  ...
  public void testDetectInvalidTopLevelMapValue() {
    ...
    Schema apply = new SchemaUpdate(currentSchema, 3).updateSchema(newSchema).apply();
  ...
  public void testDetectInvalidTopLevelMapKey() {
    ...
    new SchemaUpdate(currentSchema, 3).updateSchema(newSchema).apply();
  }
  ...
  public void testInvalidTypePromoteDoubleToFloat() {
    ...
    new SchemaUpdate(currentSchema, 1).updateSchema(newSchema).apply();
  }
  ...
  public void testAddPrimitiveToNestedStruct() {
    ...
    Schema applied = new SchemaUpdate(schema, 5).updateSchema(newSchema).apply();
  ...
  public void testReplaceListWithPrimitive() {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Cannot update existing field: aColumn of type: list<string> to primitive type: STRING");
